  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 pb-24">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-2">
            {report.offeredPrice !== null ? (
              ...
              <span className="rounded-lg bg-zinc-800/60 px-2.5 py-1 text-sm text-zinc-400">
                {lang === "ar" ? "السعر غير معروف" : "Price unknown"}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="mb-4 rounded-xl border border-amber-500/15 bg-zinc-900/60 p-5">
        ...
        <div className={`flex items-center ${report.offeredPrice !== null ? "justify-between" : "justify-center"} rounded-xl bg-zinc-800/40 p-4`}>
          {report.offeredPrice !== null && (
          ...
          {report.offeredPrice !== null && <div className="h-12 w-px bg-zinc-700" />}
          <div className="text-center">
      ...
      {/* Community Radar — REAL data with enhanced social proof.
          Requires a known offered price to compute "your price vs market". */}
      {report.offeredPrice !== null && report.communityInsights && report.communityInsights.analyzedCount >= 3 && (
            ...
            <div className="rounded-lg bg-zinc-800/40 p-3 text-center">
              <p className="text-2xl font-bold text-amber-400">
                {report.communityInsights.recentPrices?.length
                  ? Math.round((report.communityInsights.recentPrices.reduce((a, b) => a + b, 0) / report.communityInsights.recentPrices.length) / (report.offeredPrice as number) * 100)
                  : naLabel}
                %
              </p>
                  ...
                  <span
                    ...
                    className={`rounded-lg px-2.5 py-1 text-sm ${
                      p === report.offeredPrice
                        ...
                        : p < (report.offeredPrice as number)
      ...
      {/* Final Verdict — hidden when the user didn't enter an offered price,
          since there's no deal to render a verdict/recommendation on. */}
      {report.offeredPrice !== null ? (
        <div className="mb-4 rounded-xl border border-amber-500/15 bg-zinc-900/60 p-5">
          <h2 className="mb-3 flex items-center gap-2 font-serif text-lg font-bold text-amber-400">
        ...
        <div className="mb-4 rounded-xl border border-zinc-700/60 bg-zinc-900/40 p-5 text-center">
          <Info className="mx-auto mb-2 h-5 w-5 text-zinc-500" />
          <p className="text-sm text-zinc-400">
            {lang === "ar"
              ? "التوصية النهائية مش متاحة لأنك ما دخلتش سعر محدد للصفقة."
              : "A final recommendation isn't available since no specific offered price was entered."}
          </p>
        </div>
      )}
      ...
      {/* Negotiation Script — hidden when the user didn't enter an offered
          price, since there's nothing to negotiate against. */}
      {report.offeredPrice !== null ? (
        <div className="mb-4 rounded-xl border border-amber-500/15 bg-zinc-900/60 p-5">
          <h2 className="mb-3 flex items-center gap-2 font-serif text-lg font-bold text-amber-400">
        ...
        <div className="mb-4 rounded-xl border border-zinc-700/60 bg-zinc-900/40 p-5 text-center">
          ...
          <p className="text-sm text-zinc-400">
            {lang === "ar"
              ? "سكريبت التفاوض مش متاح لأنك ما دخلتش سعر محدد للصفقة."
              : "A negotiation script isn't available since no specific offered price was entered."}
          </p>
        </div>
      )}
      ...
      {report.offeredPrice !== null && typeof report.marketFairPriceMin === "number" && report.offeredPrice > report.marketFairPriceMin && (
