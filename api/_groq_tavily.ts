 ...
 * so any real store carrying it (an official brand site, a carrier store
 * like Vodafone, a local electronics chain, whatever Google itself would
 * surface) can turn up, not just the fixed Amazon/Noon/Jumia/B.TECH list.
 * Two query phrasings are combined (plain "buy" query + the site-restricted
 * "official" brand-site hint already defined per currency in
 * COUNTRY_RETAILERS) since a single Serper call only returns ~10 organic
 * hits and different phrasings surface different stores.
 */
...
function pickBroadRetailerLinks(
  ...
  for (const r of results) {
    ...
    links.push({
      ...
      url: applyAffiliateTag(r.url, domain),
    });
 ...
 * site, a carrier store, a local chain — whatever's really out there), then
 * de-duplicates by domain so the same store never appears twice. Falls back
 * to plain store-search links only when Serper genuinely returns nothing at
 * all.
 */
...
function applyAffiliateTag(url: string, domain: string): string {
  ...
  try {
    ...
    // Malformed URL (shouldn't happen since these are all built or scraped
    // as valid absolute URLs) — return untagged rather than throw.
    return url;
  }
}
...
export function buildRetailerSearchLinks(
  ...
  return visibleDomains.map((domain) => ({
    ...
    url: applyAffiliateTag(buildStoreSearchUrl(domain, product), domain),
  }));
}
...
export function pickDirectRetailerLinks(
  ...
  return visibleDomains.map((domain) => {
    ...
    const url = hit ? hit.url : buildStoreSearchUrl(domain, product);
    return {
      retailer: RETAILER_DISPLAY_NAMES[domain] || domain,
      url: applyAffiliateTag(url, domain),
    };
  });
}
