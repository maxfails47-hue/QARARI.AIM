...
function buildPrompt(opts: {
  ...
  const priceKnown = offeredPrice !== null;
...
PRODUCT: ${product}
OFFERED PRICE: ${priceKnown ? `${offeredPrice} ${currency}` : "NOT PROVIDED — the user does not know/have an offered price yet. Focus ONLY on the market/fair price analysis for this product; do NOT compare against, reference, or invent any offered price."}
...
${priceKnown
  ...
  : `- verdict: no offered price was provided, so a good/fair/bad verdict on a deal is not meaningful here — always return "fair" as a neutral placeholder; it will not be shown to the user.`}
- All prices in ${currency}.
- resaleValueRightNow: estimate what this product would sell for on the second-hand market RIGHT NOW (in ${currency}), based on brand reputation and current demand${priceKnown ? ` and the offeredPrice of ${offeredPrice} ${currency}` : " (no offered price was given, so base this on the market fair price data above instead)"}. Return null if no reliable data.
...
- finalTip / negotiationScript (MANDATORY PRICE-DIRECTION LOGIC): any specific target price you mention MUST be LOWER than offeredPrice (${offeredPrice} ${currency}) — never equal to or higher than it. The entire point of negotiating is paying LESS than the price on offer, so a target above offeredPrice is a contradiction and is never valid, regardless of where offeredPrice sits relative to marketFairPriceMin/Max.
  ...
  : `- negotiationScript / finalTip: no offered price was provided, so there is nothing to negotiate against — return short, simple bilingual strings noting that a negotiation script isn't available without an offered price (do NOT invent a target price or any offered-price comparison).`}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // offeredPrice is optional/nullable — the user may not know the price
    // ("مش عارف السعر؟" in the UI). When omitted/null we skip offered-price
    // validation entirely and rely on marketPrice-only analysis below.
    const offeredPriceProvided = offeredPrice !== null && offeredPrice !== undefined && offeredPrice !== "";
    if (!product || typeof product !== "string" || (offeredPriceProvided && Number(offeredPrice) <= 0)) {
      console.warn("[/api/analyze] Invalid input:", { product, offeredPrice });
      return res.status(400).json({ error: "invalid_input" });
    }
    console.log("[/api/analyze] Input OK. product:", product, "| offeredPrice:", offeredPriceProvided ? offeredPrice : "null (unknown)", "| currency:", currency);
    const normalizedOfferedPrice: number | null = offeredPriceProvided ? Number(offeredPrice) : null;
    ...
    if (cachedRow && cachedRow.market_data) {
      ...
      const tempPrompt = buildPrompt({ product, offeredPrice: normalizedOfferedPrice, currency, notes, purpose, duration, specs, condition, language, tier, marketPrice });
    ...
    if (reusableAiResult) {
      ...
      const dynamicPrompt = buildPrompt({
        product,
        offeredPrice: normalizedOfferedPrice,
        currency,
        notes,
        purpose,
        duration,
    ...
    // When the offered price is unknown, the verdict/negotiationScript are
    // not meaningful (there's nothing to compare against or negotiate
    // toward) — override them with simple, price-independent values rather
    // than trusting whatever the model produced, regardless of the prompt.
    if (!offeredPriceProvided) {
      decision.verdict = "fair";
      decision.negotiationScript = {
        ar: "سكريبت التفاوض غير متاح لأنك لم تدخل سعرًا محددًا للصفقة.",
        en: "A negotiation script isn't available since no specific offered price was entered.",
      };
      if (decision.negotiationScriptVariants) {
        delete decision.negotiationScriptVariants;
      }
    }

    const marketFairPriceMid: number | null = normalizedMarket.mid;
    const moneySaved = !offeredPriceProvided || marketFairPriceMid === null
      ? null
      : Math.max(0, marketFairPriceMid - (normalizedOfferedPrice as number));
    ...
    try {
      // product_price_events.offered_price is NOT NULL — an unknown price
      // has nothing meaningful to log here, so we skip the event entirely
      // but still surface any existing community insights below.
      if (offeredPriceProvided) {
        await admin.from("product_price_events").insert({
          cache_key: cacheKey,
          offered_price: normalizedOfferedPrice,
          currency,
        });
      }
